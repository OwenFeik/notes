...
\[\{\sqrt[^n]{s}e^{i\frac{1}{n}(\alpha + 2k\pi)} | k \in [0, n) \cap \mathbb{N}\}\]
...
For \(w = re^{i\theta} \in \mathbb{C} \backslash \{0\}\), the root finding formula gives two solutions of \(z^2 = w\):
...
\section*{Polynomials}
Real polynomials are those with coefficients exclusively in \(\mathbb{R}\), while complex polynomials
extend this bank of coefficients to include all of \(\mathbb{C}\). All real polynomials are complex 
...
Any complex polynomial of degree \(n\) can be factorised into \(n\) linear factors over \(\mathbb{C}\).
\[p(z) = a(z - \alpha_1)(z - \alpha_2) ... (z - \alpha_n)\]
Thus, there is an \(x\) intercept at each value of \(\alpha\). For a polynomial of degree \(n\),
there at \(\leq n\) roots in \(\mathbb{C}\).
...
\[f: \mathbb{R} \rightarrow \mathbb{R}, f(x) = \frac{x + 1}{x^2 + 4}\]
is a valid function. However,
\[h: \mathbb{C} \rightarrow \mathbb{C}, h(z) = \frac{x + 1}{x^2 + 4}\]
is not, as the rule \(h(z)\) is undefined for some \(z \in \mathbb{C}\).
To make it a valid function, we could change the domain to
\(\mathbb{C} \backslash \{2i, -2i\}\).
When a function has a domain and codomain as subsets of \(\mathbb{R}\), we can
...
\bigskip
\textit{Surjective} functions have a minimal codomain, i.e. the range is equal to the codomain
or \(f(A) = B\). 
...
a partner in \(B\) with no excess elements in \(B\).
...
\(\tan(x)\) is undefined, (\(\{k\pi + \frac{\pi}{2} | k \in \mathbb{z}\}\)) are defined 
